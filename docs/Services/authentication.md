...
The system will perform User Authorization for protected resources
after[Security Token Validation](#security-token-validation). Depending on the service, additional user authorization
checks may apply (see below).
...
1. For all requests, the system will perform base user authorization.
   ...
   1.1. System will check if the User has Site Manager (Industry) permissions for any Site. If the User does not have
   permission the System will stop processing and generate the following error:
